...
function metaFor(pathname: string): PageMeta {
  ...
  const defaults: Record<string, Omit<PageMeta, "type">> = { "/": { title: "생활을 정리하는 한결 쉬운 길잡이", description: "생활에 바로 필요한 돈·혜택, 건강·돌봄, 일상·디지털, 주거·안전 정보를 읽기 쉬운 순서로 안내합니다." }, "/articles": { title: "최신 글", description: "한결생활 가이드에서 새로 정리하고 검토한 생활 정보 글을 확인하세요." }, "/search": { title: "사이트 검색", description: "한결생활 가이드의 생활 정보 글을 검색하세요." }, "/calculators": { title: "생활 계산기", description: "생활비와 고정비처럼 일상 판단에 필요한 간단한 계산 도구를 제공합니다." }, "/about": { title: "사이트 소개와 운영 원칙", description: "한결생활 가이드의 편집 기준과 콘텐츠 운영 원칙을 확인하세요." }, "/contact": { title: "문의 안내", description: "한결생활 가이드의 콘텐츠 정정, 이용 문의, 제휴 요청을 위한 안내입니다." }, "/privacy": { title: "개인정보처리방침", description: "한결생활 가이드의 개인정보 처리 기준과 이용자 권리 안내입니다." }, "/disclaimer": { title: "면책 안내", description: "한결생활 가이드에서 제공하는 생활 정보의 이용 범위와 확인이 필요한 사항을 안내합니다." }, "/404": { title: "페이지를 찾을 수 없습니다", description: "요청하신 페이지를 찾을 수 없습니다. 한결생활 가이드에서 필요한 생활 정보를 다시 찾아보세요." } };
...
const routes = ["/", "/articles", "/search", "/calculators", "/about", "/contact", "/privacy", "/disclaimer", "/404", ...categories.map((category) => `/category/${category.slug}`), ...articles.map((article) => `/articles/${article.slug}`)];
